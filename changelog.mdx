...
<Update label="2026-06-26" description="NetFluss 2.4" tags={["feature","improvement"]}>
  ## What's new

  - Added a built-in **VPN client** supporting OpenVPN, WireGuard, and IKEv2/IPsec/L2TP protocols, so you can connect without installing provider-specific VPN software.
  - Enabled **profile import** from single `.ovpn` or `.conf` files, folders of configs, or `.zip` bundles (e.g. a provider's router-profile pack); each config becomes a separately selectable server endpoint.
  - Added **VPN controls in the popover** including a profile picker, server picker, status display, connect/disconnect button, exit-node country with flag, and the assigned tunnel IP address.
  - Supported **per-VPN DNS presets** that apply one of your existing DNS presets while connected and restore the previous DNS automatically on disconnect.
  - Added **auto-reconnect with exponential backoff** so a dropped tunnel is re-established automatically; a deliberate disconnect cancels the cycle.
  - Added a **connect-on-launch** option so a chosen profile connects automatically when NetFluss starts; only one profile can have this enabled at a time.
  - Provided full **profile management** in Preferences → VPN — import, rename, reorder, and delete profiles; credentials are stored securely in the macOS Keychain.

  ## Improvements

  - Reworked background sampling to **much lower energy use** by replacing the heavyweight `nettop` binary with lightweight `netstat` snapshots, returning energy impact to normal menu-bar-app levels.
  - Improved **download bandwidth accuracy on macOS 26.5** where the primary adapter's inbound byte counter can be frozen, now measured correctly without extra CPU cost.
  - Added a new **"System default" menu bar color** option that follows the menu bar appearance automatically, staying legible whether the wallpaper is light or dark (credits @mvanhorn).
</Update>

<Update label="2026-05-22" description="NetFluss 2.3" tags={["feature","improvement","bugfix"]}>
  ## What's new

  - Added a **Wi-Fi manager in the popover** so you can switch Wi-Fi networks directly from the menu bar without opening System Settings.
  - Supported **pinning SSIDs** to the top of the list; pinned networks that go out of range stay visible with a "Not available" label and re-trigger a targeted scan when tapped.
  - Saved **Wi-Fi passwords to the macOS Keychain via the privileged helper** so joined networks are written into macOS's Known Networks list and reused by the system Wi-Fi menu even when NetFluss isn't running.
  - Showed **per-row details** for each network (band, channel, RSSI, security type, BSSID) via an (i) button, and added an optional "only show the strongest N" cap to keep the list short in crowded environments.
  - Introduced **reorderable and toggleable popover sections** in Preferences → Appearance, letting you drag the seven segments (Download/Upload, Network adapters, Network flow, DNS, Router, Wi-Fi Networks, Top Apps) into any order and toggle visibility per section.
  - Added a new **Wi-Fi preferences pane** with the Wi-Fi switcher toggle, strongest-N cap, and Location Services explanation.
  - Added **system-access shortcuts** in Preferences → General with one-click buttons to Grant Location Access and Install Privileged Helper.

  ## Improvements

  - Added a hidden **"Copy Network Diagnostics…"** action behind Option + right-click of the menu bar icon for debugging.
  - Added a **15-second defensive timeout** on every privileged-helper XPC call so the DNS switcher and Wi-Fi credential saves can no longer hang the popover.
  - Localised all new UI strings (and a handful of pre-existing ones) into German, Simplified Chinese, and Traditional Chinese.

  ## Bug fixes

  - Fixed the **macOS 26.5 frozen&#x20;****`ifi_ibytes`****&#x20;download counter** on managed-profile Macs by substituting a per-process inbound rate from `nettop`, so the menu-bar download number and Bandwidth Statistics history record correctly.
</Update>

<Update label="2026-04-21" description="NetFluss 2.2" tags={["feature","ui","improvement"]}>
  ## What's new

  - Redesigned **Preferences into clear panes** — General, Adapters, Statistics, Appearance, Top Apps, DNS, and Router — replacing the single scrolling list so settings are easier to find.
  - Added **language support** for English, German, Simplified Chinese, and Traditional Chinese; follows the macOS system language by default and can be changed manually in Preferences → General.
  - Added **OPNsense router bandwidth support**, grouped with Fritz!Box, UniFi, and OpenWRT in the new Router pane.
  - Added a **None option for IP display** so the popover can hide IP address information entirely.

  ## Improvements

  - Improved adapter statistics accounting to avoid inflated totals from invalid interface counter deltas.
  - Cleaned up the custom date range controls in the bandwidth statistics window.

  ## Other highlights

  - Updated release packaging so **localization files are included in the signed app bundle** (credits @oezkanyilmaz).
</Update>

<Update label="2026-04-13" description="NetFluss 2.1" tags={["feature","improvement","bugfix"]}>
  ## What's new

  - Switched **DNS changes to a bundled privileged helper** that uses a one-time macOS approval flow, after which DNS switching and Ethernet resets no longer need repeated password prompts.
  - Based **active network service detection on the real macOS service mapping**, fixing unreliable switching when the service name differs from the hardware port.
  - Expanded **custom DNS presets to support up to four DNS servers**.
  - Added a **pin button** that turns the menu bar popup into a movable floating window so NetFluss can stay open like a live widget.
  - Made **Top Apps update live** while the popup or pinned window is open.

  ## Improvements

  - Made **OpenWRT support more robust** with HTTPS-first probing, self-signed certificate handling, better ubus error reporting, and WAN discovery fallback when `network.interface.wan` is not the active uplink.
  - Made **Fritz!Box polling more resilient** to transient request failures, keeping the last good sample visible instead of dropping into an error state.
  - Allowed **Speed Test History to be opened without starting a new test**.
  - Added support for **notes on saved speed test results**.
  - Made history timestamps more compact and locale-aware.
  - Sped up popup Top Apps sampling for more responsive updates.
